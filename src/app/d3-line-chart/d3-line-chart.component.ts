...
export class D3LineChartComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
    if (this.parseDate &&  this.chartProps) {
      this.updateChart();
  ...
  updateChart() {
    ...
    this.chartProps.x.domain(d3.extent(this.dataSet, function (d) {
      if (d.date instanceof Date) {
        return d.date.getTime();
      }
    }));

    this.chartProps.y.domain([0, d3.max(this.dataSet, function (d) { return Math.max(d.close, d.open); })]);
    ...
    this.chartProps.svg.select('.line.line1') // update the line
      .attr('d', this.chartProps.valueline(this.dataSet));

    this.chartProps.svg.select('.line.line2') // update the line
      .attr('d', this.chartProps.valueline2(this.dataSet));
  ...
  buildChart() {
    ...
    // Define the line
    var valueline = d3.line<ChartValue>()
      .x(function (d) {
        if (d.date instanceof Date) {
    ...
    // Define the line
    var valueline2 = d3.line<ChartValue>()
      .x(function (d) {
        if (d.date instanceof Date) {
          return _this.chartProps.x(d.date.getTime());
        }
    ...
    this.chartProps.x.domain(
      d3.extent(_this.dataSet, function (d) {
        if (d.date instanceof Date)
          return (d.date as Date).getTime();
      }));
    this.chartProps.y.domain([0, d3.max(this.dataSet, function (d) {
    ...
    svg.append('path')
      ...
      .style('stroke', 'green')
      .style('fill', 'none')
      .attr('d', valueline2(_this.dataSet));
    ...
    svg.append('path')
      ...
      .style('stroke', 'black')
      .style('fill', 'none')
      .attr('d', valueline(_this.dataSet));
